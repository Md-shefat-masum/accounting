                                                                        ...
                                                                        <div class="dropdown">
                                                                            ...
                                                                            <ul class="dropdown-menu A54VNK-pi-c">
                                                                                <li class="disabled" v-for="recipient in recipients" :key="recipient.id">
                                                                                    <span @click="form.recipient = recipient.first_name+' '+recipient.last_name" class="d-block p-1" style="cursor:pointer">
    ...
    export default {
        ...
        methods: {
            setInfo: function(){
                ...
                this.form.expected_delivery = this.expiration_date;
            },
            get_note: function(){
                let id = this.$route.params.id;
                var that = this;
                this.form.get('/api/receivingnotes/' + id).then(function (response) {
                    that.form.fill(response.data.notes);
                    that.selected_products = response.data.selected_products;
                    that.loaded = true;
                    that.calculateTotal();
                    // console.log(response.data.selected_products);
                    that.purchase_logs = response.data.notes.purchase_logs;
                    setTimeout(() => {
                        that.get_suppliers_data(response.data.notes.supplier_id);
                        if(that.form.status === 'billed'){
            ...
            get_suppliers_data:function(id){
            ...
            getCode: function (Quote) {
                var that = this;
                this.form.get('/api/get-latest-code-id/receiving_notes/Rec').then(function (response) {
            ...
            update: function() {
                ...
                this.form.put('/api/receivingnotes/'+this.$route.params.id).then(() => {
                    this.form.reset();
                    ...
                    this.$router.push({name: 'ReceivingNotes'})
                }).catch((err) => {
